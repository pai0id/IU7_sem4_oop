...
{
    public:
    SqliteReaderException(const char *filename,
...
{
    public:
    SqliteNoFileException(const char *filename,
                     const size_t line, const char *class_name,
                     const char *method_name,
                     const char *info = "Can't open sqlite file exception.")
        : SqliteReaderException(filename, line, class_name, method_name, info){};
};

class SqliteReadException : public SqliteReaderException
{
    public:
    SqliteReadException(const char *filename,
                     const size_t line, const char *class_name,
                     const char *method_name,
